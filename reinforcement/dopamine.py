...
class DopamineController:
    ...
    def register_event(self, event_type: str, magnitude: float = 1.0) -> None:
        """
    ...
    def update(self, motion_flow: float, is_obstacle: Optional[bool] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        # Flow threshold calibrated to 60x60 gradient optical flow (~0.00002)
        is_stationary = (motion_flow < 0.00002)
        if is_obstacle is not None:
            if is_stationary and is_obstacle:
                self.stationary_steps += 1
            else:
                self.stationary_steps = max(0, self.stationary_steps - 2)
                if not is_obstacle and motion_flow >= 0.00005:
                    self.register_event("forward", magnitude=0.12)
        else:
            if is_stationary:
                self.stationary_steps += 1
            else:
